...
vientos_mayores_63km(X):-nl, write(X), write('existe vientos mayores a 63km/h?[s/n]: '), read(R),R=s.
vientos_mayores_116km(X):-nl, write(X), write('existe vientos mayores a 116km/h?[s/n]: '), read(R),R=s.

% las reglas 

depresion_tropical(X):-vientos_menores_63km/h (X),nubosidad(X),precipitaciones(X),aumento_marea_oleaje(X),.
tormenta_tropical(X):-vientos_mayores_63km/h(X),nubosidad(X),precipitaciones(X),aumento_marea_oleaje(X),.
